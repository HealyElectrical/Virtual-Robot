...
private:
   ...
   // pointer to the camera used in AR mode for marker poses
   CCameraReal* marker_cam_ = nullptr;
   ////////////////lab 7 trajectory function /////////////////////
...
public:
   ...
   void start_marker_cycle(double q1_deg, double q2_deg,
      double q3_deg, double d3_m);

   void stop_marker_cycle();

   bool step_marker_cycle(double& q1_deg, double& q2_deg,
      double& q3_deg, double& d3_m);

   bool plan_marker_leg(int marker_id,
      double q1_start, double q2_start,
      double q3_start, double d3_start);



   void start_ctraj_cycle(CCameraReal& cam,
      double& q1_deg, double& q2_deg,
      double& q3_deg, double& d3_m);

   void tick_ctraj_cycle(CCameraReal& cam,
      double& q1_deg, double& q2_deg,
      double& q3_deg, double& d3_m);

   void stop_ctraj_cycle();
...
private:
   ...
   int  traj_step_ = 0;
   int  traj_nsteps_ = 0;
   std::vector<double> traj_q1_;
   std::vector<double> traj_q2_;
   ...
   bool step_ctraj(double& q1_deg, double& q2_deg,
      double& q3_deg, double& d3_m);
   bool marker_cycle_running_ = false;

   enum class MarkerCycleState
   {
      Idle,
      MoveToMarker,
      HoldAtMarker
   ...
   MarkerCycleState marker_cycle_state_ = MarkerCycleState::Idle;

   // IDs we will visit in a loop: 50 -> 60 -> 70 -> 50 -> ...
   int marker_ids_[3] = { 50, 60, 70 };
   int marker_index_ = 0;          // 0,1,2 -> current target in marker_ids_

   // joint-space trajectory for current leg (current pose -> current marker)
   std::vector<double> mc_q1_;
   std::vector<double> mc_q2_;
   std::vector<double> mc_q3_;
   std::vector<double> mc_d3_;

   int mc_step_ = 0;
   int mc_nSteps_ = 0;

   // how many frames to dwell at each marker before moving on
   int mc_hold_counter_ = 0;
   int mc_hold_frames_ = 60;   // about 1 second if you run at ~60 fps

   bool marker_to_target_from_id(int marker_id,
      double& xt, double& yt,
      double& zt, double& theta_deg);

   // --- Simple ctraj-based marker cycle: 50 -> 60 -> 70 -> ...
   bool ctraj_cycle_running_ = false;
   int  ctraj_cycle_idx_ = 0;                // index into ctraj_cycle_markers_
   int  ctraj_cycle_hold_counter_ = 0;
   int  ctraj_cycle_hold_frames_ = 60;       // frames to dwell at each marker
   int  ctraj_cycle_markers_[3] = { 50, 60, 70 };
