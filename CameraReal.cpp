...
// Simple exponential smoothing for the ChArUco board pose.
// Keeps a static previous pose and blends toward the new one.
static void smooth_board_pose(const cv::Vec3d& r_new, const cv::Vec3d& t_new,
   cv::Vec3d& r_smooth, cv::Vec3d& t_smooth)
{
   static bool first = true;
   static cv::Vec3d r_prev, t_prev;

   // 0 < alpha <= 1.0
   // - smaller alpha = more smoothing, more lag
   // - larger alpha  = less smoothing, more jitter
   const double alpha = 0.1; // you can try 0.05, 0.2, etc.
   ...
   {
      r_prev = (1.0 - alpha) * r_prev + alpha * r_new;
      t_prev = (1.0 - alpha) * t_prev + alpha * t_new;
   }
...
bool CCameraReal::detectBoardPose(cv::Mat& frame) {
   ...
   // Pose of BOARD in CAMERA frame (raw)
   cv::Vec3d rvec_raw, tvec_raw;
   if (!cv::solvePnP(
      objPts, imgPts,
      _cam_webcam_intrinsic, _cam_webcam_dist_coeff,
      rvec_raw, tvec_raw, false, cv::SOLVEPNP_ITERATIVE))
   {
      have_pose = false;
      return false;
   }

   // Smooth the pose to reduce jitter
   cv::Vec3d rvec_smooth, tvec_smooth;
   smooth_board_pose(rvec_raw, tvec_raw, rvec_smooth, tvec_smooth);
...
bool CCameraReal::draw_cube_on_marker(cv::Mat& frame, int marker_id, float markerLen, float cubeH)
{
   if (frame.empty() || _cam_webcam_intrinsic.empty())
      return false;

   // Per-marker pose cache (static so it persists across calls)
   static bool      have_cache[256] = { false };     // assuming IDs 0..255
   static cv::Vec3d last_rvec_arr[256];
   static cv::Vec3d last_tvec_arr[256];

   const double SMOOTH_ALPHA = 0.05;  // 0..1, higher = snappier, lower = smoother
   ...
   auto draw_cube_with_pose = [&](const cv::Vec3d& rvec, const cv::Vec3d& tvec) -> bool
      {
         ...
         std::vector<cv::Point2f> imgPts;
         cv::projectPoints(cube3d, rvec, tvec,
            _cam_webcam_intrinsic, _cam_webcam_dist_coeff,
            imgPts);
         if (imgPts.size() != 8)
            return false;
         ...
         auto L = [&](int a, int b)
            {
               cv::line(frame, imgPts[a], imgPts[b],
         ...
         // Optional: small axes at marker center
         cv::drawFrameAxes(frame,
            _cam_webcam_intrinsic, _cam_webcam_dist_coeff,
            rvec, tvec, 0.5f * markerLen);
         return true;
      };

   // For comparing orientations (angle between rotations)
   auto rot_diff = [](const cv::Vec3d& r1, const cv::Vec3d& r2) -> double
      {
         cv::Mat R1, R2;
         cv::Rodrigues(r1, R1);
   ...
   // 1) Detect markers in this frame
   cv::aruco::Dictionary dict =
      cv::aruco::getPredefinedDictionary(cv::aruco::DICT_6X6_250);
   cv::aruco::DetectorParameters detParams;
   cv::aruco::ArucoDetector detector(dict, detParams);
   ...
   // If this marker is not visible, do nothing
   if (idx < 0)
      return false;

   const std::vector<cv::Point2f>& c = corners[idx];
   if (c.size() != 4 || markerLen <= 0.0f)
      return false;

   // 2) Estimate pose of this marker using IPPE_SQUARE
   std::vector<cv::Point3f> obj = {
      cv::Point3f(-markerLen / 2.0f,  markerLen / 2.0f, 0.0f),
      cv::Point3f(markerLen / 2.0f,  markerLen / 2.0f, 0.0f),
      cv::Point3f(markerLen / 2.0f, -markerLen / 2.0f, 0.0f),
   ...
   // Choose a base candidate: positive Z and lowest reprojection error
   int best = -1;
   double bestErr = 1e100;
   for (size_t i = 0; i < rvecs.size(); ++i)
   {
   ...
   // 3) Use per-marker cache to:
   //    - Pick the candidate closest to previous orientation (avoid flips)
   //    - Smooth translation to reduce jitter
   int mid = marker_id;
   if (mid >= 0 && mid < 256 && have_cache[mid])
   {
      // Use history ONLY to pick the correct IPPE solution (avoid flips),
      // but DO NOT smooth tvec when marker is visible.
      if (rvecs.size() > 1)
      {
         int alt = (best == 0 ? 1 : 0);
         cv::Vec3d alt_r(
         ...
         double d_best = rot_diff(last_rvec_arr[mid], rvec);
         double d_alt = rot_diff(last_rvec_arr[mid], alt_r);

         if (d_alt + 1e-6 < d_best)
         {
            rvec = alt_r;
            tvec = cv::Vec3d(
               tvecs[alt].at<double>(0, 0),
               tvecs[alt].at<double>(1, 0),
               tvecs[alt].at<double>(2, 0)
            );
         }
      }

      // IMPORTANT: no low-pass filtering of tvec here.
      // We want the cube to match this frame's marker exactly.
      // If we smooth, the cube will lag the image and appear misaligned.
   }


   // 4) Draw the cube using the final pose
   bool drawn = draw_cube_with_pose(rvec, tvec);
   if (drawn && mid >= 0 && mid < 256)
   {
      have_cache[mid] = true;
      last_rvec_arr[mid] = rvec;
      last_tvec_arr[mid] = tvec;
   }
