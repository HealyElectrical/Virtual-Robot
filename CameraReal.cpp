...
bool CCameraReal::draw_cube_on_marker(cv::Mat& frame, int marker_id, float markerLen, float cubeH)
{
   ...
   const double SMOOTH_ALPHA = 0.1;  // 0..1, higher = snappier, lower = smoother
   ...
   {
      // If we have a second candidate, see if it is closer to the old pose
      if (rvecs.size() > 1)
      {
         int alt = (best == 0 ? 1 : 0);
         cv::Vec3d alt_r(
      ...
      // Low-pass filter the translation
      tvec = SMOOTH_ALPHA * tvec + (1.0 - SMOOTH_ALPHA) * last_tvec_arr[mid];
   }
