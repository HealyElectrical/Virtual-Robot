   ...
   double& q1_deg, double& q2_deg, double& q3_deg, double& d3_m)
{
   ...
   if (view_mode_ == ViewMode::AR)
   {
      ...
      // Remember which camera we are using for marker poses
      marker_cam_ = &cam;
      ...
      // 4) Draw cube on marker 50 FIRST
      const float kMarkerLen = 0.02042f;
      const float kCubeH = 0.030f;
      (void)cam.draw_cube_on_marker(_canvas, 50, kMarkerLen, kCubeH);

      // 5) Overlay ArUco marker IDs
      cam.draw_marker_ids(_canvas);
      ...
      // 6b) Lab 7: cycle 50 -> 60 -> 70
      if (cvui::button(_canvas, 10, 90, 220, 30,
         "Lab7: cycle 50-60-70"))
      {
         start_marker_cycle(q1_deg, q2_deg, q3_deg, d3_m);
      }


      // 7) Advance any animations (FK / IK / jtraj / ctraj / marker cycle)
      tick_animations(q1_deg, q2_deg, q3_deg, d3_m);

      // 8) Draw SCARA in world
      draw_scara_world(cam, q1_deg, q2_deg, q3_deg, d3_m);
   ...
   else
   {
      ...
      // Advance any running animations (marker-cycle won't run if we're in Virtual)
      tick_animations(q1_deg, q2_deg, q3_deg, d3_m);
   ...
   double& q1_deg, double& q2_deg, double& q3_deg, double& d3_m)
{
   ...
   if (view_mode_ == ViewMode::AR)
   {
      // --- simple state machine for ctraj cycling 50->60->70 ---
      static bool ctraj_cycle_running = false;
      static int  ctraj_marker_index = 0;    // 0,1,2 into markers[]
      static int  ctraj_hold_counter = 0;

      const int markers[3] = { 50, 60, 70 };
      const int hold_frames = 60;        // ~1 second at 60 fps
      ...
      // 6) Lab 7 button: single ctraj to marker 50
      if (cvui::button(_canvas, 10, 50, 220, 30,
         "Lab7: ctraj to marker 50"))
      {
         // Stop any ongoing cycle, fire one-shot ctraj
         ctraj_cycle_running = false;
         start_ctraj_to_marker(cam, 50, q1_deg, q2_deg, q3_deg, d3_m);
      }

      // 6b) Lab 7: cycle 50 -> 60 -> 70 using ctraj for each leg
      if (cvui::button(_canvas, 10, 90, 220, 30,
         "Lab7: cycle 50-60-70"))
      {
         ctraj_cycle_running = true;
         ctraj_marker_index = 0;
         ctraj_hold_counter = 0;

         // Start first leg to marker[0] (50)
         start_ctraj_to_marker(cam, markers[ctraj_marker_index],
            q1_deg, q2_deg, q3_deg, d3_m);

         // If we could not see marker 50, abort cycle
         if (!ctraj_running_) {
            std::cout << "[cycle] Marker " << markers[ctraj_marker_index]
               << " not visible, not starting cycle.\n";
            ctraj_cycle_running = false;
         }
      }
      ...
      // 7b) Manage the ctraj-based marker cycle
      if (ctraj_cycle_running)
      {
         if (ctraj_running_) {
            // current leg still in flight; do nothing
         }
         else {
            // ctraj finished, we are "at" the marker; hold for a bit
            ctraj_hold_counter++;

            if (ctraj_hold_counter >= hold_frames)
            {
               ctraj_hold_counter = 0;

               // move to next marker in {50,60,70}
               ctraj_marker_index = (ctraj_marker_index + 1) % 3;

               std::cout << "[cycle] Moving to marker "
                  << markers[ctraj_marker_index] << "\n";

               start_ctraj_to_marker(cam, markers[ctraj_marker_index],
                  q1_deg, q2_deg, q3_deg, d3_m);

               // If we cannot see this new marker, stop the cycle
               if (!ctraj_running_) {
                  std::cout << "[cycle] Marker " << markers[ctraj_marker_index]
                     << " not visible, stopping cycle.\n";
                  ctraj_cycle_running = false;
               }
            }
         }
   ...
   double& q1_deg, double& q2_deg, double& q3_deg, double& d3_m)
{
   ...
   if (view_mode_ == ViewMode::AR)
   {
      // --- ctraj-based cycle state: 50 -> 60 -> 70 ---
      // These are static so they persist between frames.
      static bool ctraj_cycle_running = false;
      static int  ctraj_marker_index = 0;   // 0,1,2 into markers[]
      static int  ctraj_hold_counter = 0;
      ...
      // 4) Draw cube on marker 50 FIRST (your original Lab 6/7 behaviour)
      (void)cam.draw_cube_on_marker(_canvas, 50, kMarkerLen, kCubeH);
      ...
      if (cvui::button(_canvas, 10, 50, 220, 30,
         "Lab7: ctraj to marker 50"))
      {
         // Stop any ongoing cycle, then do the one-shot move
         ctraj_cycle_running = false;

         std::cout << "[ctraj] One-shot ctraj to marker 50\n";
         start_ctraj_to_marker(cam, 50, q1_deg, q2_deg, q3_deg, d3_m);
      }
      ...
      if (cvui::button(_canvas, 10, 90, 220, 30,
         "Lab7: cycle 50-60-70"))
      {
         // (Re)start the cycle from marker 50
         ctraj_cycle_running = true;
         ctraj_marker_index = 0;
         ctraj_hold_counter = 0;

         int id = markers[ctraj_marker_index];
         std::cout << "[cycle] Starting cycle at marker " << id << "\n";

         // Just call the same ctraj function you know works for 50.
         start_ctraj_to_marker(cam, id, q1_deg, q2_deg, q3_deg, d3_m);

         // If for some reason the ctraj didn?t start, stop the cycle
         if (!ctraj_running_) {
            std::cout << "[cycle] start_ctraj_to_marker failed for marker "
               << id << ", not starting cycle.\n";
            ctraj_cycle_running = false;
         }
      }
      ...
      {
         if (ctraj_running_) {
            // current ctraj leg still in flight; nothing to do
         }
         else {
            // ctraj finished, we are "at" the current marker; hold for a bit
            ctraj_hold_counter++;
            ...
            if (ctraj_hold_counter >= hold_frames)
            {
               ...
               std::cout << "[cycle] Moving to marker " << id << "\n";

               // Fire a new ctraj leg
               start_ctraj_to_marker(cam, id, q1_deg, q2_deg, q3_deg, d3_m);

               // If we cannot start this leg (e.g., marker pose not available),
               // stop the cycle so you don?t get stuck in a weird state.
               if (!ctraj_running_) {
                  std::cout << "[cycle] start_ctraj_to_marker failed for marker "
                     << id << ", stopping cycle.\n";
                  ctraj_cycle_running = false;
   ...
   double& q1_deg, double& q2_deg, double& q3_deg, double& d3_m)
{
   ...
   if (view_mode_ == ViewMode::AR)
   {
      ...
      const int markers[3] = { 50, 60, 70 };
      const int hold_frames = 5;          // ~1 second at 60 fps
      ...
      // 4) Draw cubes on markers 50 and 60
      const float kMarkerLen = 0.02042f;
      const float kCubeH = 0.030f;

      (void)cam.draw_cube_on_marker(_canvas, 50, kMarkerLen, kCubeH);
      (void)cam.draw_cube_on_marker(_canvas, 60, kMarkerLen, kCubeH);
      (void)cam.draw_cube_on_marker(_canvas, 70, kMarkerLen, kCubeH);

      // 5) Overlay ArUco marker IDs (so you see 50/60/70)
      cam.draw_marker_ids(_canvas);

      // 6) Debug text: board-frame target pose for each marker
      auto debug_marker_pose = [&](int marker_id, int y)
         {
            cv::Vec3d rvec_BM, tvec_BM;
            if (cam.get_marker_pose_in_board(marker_id, rvec_BM, tvec_BM))
            {
               // Convert to WORLD (board) target that your ctraj uses
               const double x = tvec_BM[0];
               const double yB = tvec_BM[1];
               const double z = pedestalZ_ + 0.11 + (tvec_BM[2] + tool_z_offset_);

               // Yaw angle from marker orientation
               cv::Mat Rbm;
               ...
               char buf[256];
               std::snprintf(buf, sizeof(buf),
                  "ID %d: x=%.3f y=%.3f z=%.3f th=%.1f",
                  marker_id, x, yB, z, th_deg);

               cv::putText(_canvas, buf, cv::Point(10, y),
                  cv::FONT_HERSHEY_SIMPLEX, 0.5, YELLOW, 1);
            }
            else
            {
               char buf[64];
               std::snprintf(buf, sizeof(buf), "ID %d: not visible", marker_id);
               cv::putText(_canvas, buf, cv::Point(10, y),
                  cv::FONT_HERSHEY_SIMPLEX, 0.5, RED, 1);
            }
         };

      // Show pose debug for 50, 60, 70 near the top-left
      debug_marker_pose(50, 20);
      debug_marker_pose(60, 40);
      debug_marker_pose(70, 60);

      // 7) Button: single ctraj to marker 50
      if (cvui::button(_canvas, 10, 80, 220, 30,
         "Lab7: ctraj to marker 50"))
      {
         ctraj_cycle_running = false; // stop any cycle
         start_ctraj_to_marker(cam, 50, q1_deg, q2_deg, q3_deg, d3_m);
      }

      // 7b) Button: start cycle 50->60->70 using ctraj
      if (cvui::button(_canvas, 10, 120, 220, 30,
         "Lab7: cycle 50-60-70"))
      {
         ctraj_cycle_running = true;
         ctraj_marker_index = 0;
         ...
         // Start first leg to markers[0] (50)
         start_ctraj_to_marker(cam, markers[ctraj_marker_index],
            q1_deg, q2_deg, q3_deg, d3_m);
      ...
      // 8) Advance any animations (FK / IK / jtraj / ctraj)
      tick_animations(q1_deg, q2_deg, q3_deg, d3_m);

      // 8b) Manage the ctraj-based marker cycle
      if (ctraj_cycle_running)
      {
         if (ctraj_running_) {
            // current leg still in flight; nothing to do here
         }
         else {
            // ctraj finished, we are "at" the marker; hold for a bit
            ctraj_hold_counter++;
...
void CRobot::tick_animations(double& q1_deg, double& q2_deg,
   double& q3_deg, double& d3_m)
{
   // Existing animation bits (keep whatever you already had, but
   // make sure the function signature matches this one).

   // Example structure (adapt to your existing code):
   ...
   if (traj_running_) {
      if (!step_joint_traj(q1_deg, q2_deg, q3_deg, d3_m)) {
         traj_running_ = false;
      }
   }

   if (ctraj_running_) {
      if (!step_ctraj(q1_deg, q2_deg, q3_deg, d3_m)) {
         ctraj_running_ = false;
      }
   }

   if (marker_cycle_running_) {
      if (!step_marker_cycle(q1_deg, q2_deg, q3_deg, d3_m)) {
         marker_cycle_running_ = false;
      }
   }
}
...
bool CRobot::plan_marker_leg(int marker_id,
   double q1_start, double q2_start,
   double q3_start, double d3_start)
{
   double xt, yt, zt, theta_deg;
   if (!marker_to_target_from_id(marker_id, xt, yt, zt, theta_deg)) {
      std::printf("Marker %d not found, stopping marker cycle.\n", marker_id);
      return false;
   }

   double q1_t, q2_t, d3_t, q4_t;
   if (!ikine(xt, yt, zt, theta_deg, q1_t, q2_t, d3_t, q4_t, elbow_up_toggle_)) {
      std::printf("IK failed for marker %d, stopping marker cycle.\n", marker_id);
      return false;
   }

   // distance-based step count
   const double dx = q1_t - q1_start;
   const double dy = q2_t - q2_start;
   const double dz = q4_t - q3_start;
   const double dd = d3_t - d3_start;
   const double dist = std::sqrt(dx * dx + dy * dy + dz * dz + dd * dd);

   int nSteps = static_cast<int>(300.0 / (1.0 + dist));
   nSteps = std::max(60, std::min(300, nSteps));

   mc_q1_ = jtraj_scalar(q1_start, q1_t, 0.0, 0.0, nSteps);
   mc_q2_ = jtraj_scalar(q2_start, q2_t, 0.0, 0.0, nSteps);
   mc_q3_ = jtraj_scalar(q3_start, q4_t, 0.0, 0.0, nSteps);
   mc_d3_ = jtraj_scalar(d3_start, d3_t, 0.0, 0.0, nSteps);

   mc_step_ = 0;
   mc_nSteps_ = nSteps;

   marker_cycle_state_ = MarkerCycleState::MoveToMarker;
   return true;
}


void CRobot::start_marker_cycle(double q1_deg, double q2_deg,
   double q3_deg, double d3_m)
{
   ...
   marker_cycle_running_ = true;
   marker_cycle_state_ = MarkerCycleState::Idle;
   marker_index_ = 0;
   mc_hold_counter_ = 0;

   if (!plan_marker_leg(marker_ids_[marker_index_],
      q1_deg, q2_deg, q3_deg, d3_m))
   {
      marker_cycle_running_ = false;
   }
...
bool CRobot::step_marker_cycle(double& q1_deg, double& q2_deg,
   double& q3_deg, double& d3_m)
{
   if (!marker_cycle_running_)
      return false;

   switch (marker_cycle_state_)
   {
   case MarkerCycleState::MoveToMarker:
      if (mc_step_ < mc_nSteps_) {
         q1_deg = mc_q1_[mc_step_];
         q2_deg = mc_q2_[mc_step_];
         q3_deg = mc_q3_[mc_step_];
         d3_m = mc_d3_[mc_step_];
         ++mc_step_;
      }
      else {
         marker_cycle_state_ = MarkerCycleState::HoldAtMarker;
         mc_hold_counter_ = 0;
      }
      break;

   case MarkerCycleState::HoldAtMarker:
      ++mc_hold_counter_;
      if (mc_hold_counter_ >= mc_hold_frames_) {
         // next marker: 50 -> 60 -> 70 -> 50 ...
         marker_index_ = (marker_index_ + 1) % 3;
...
void CRobot::stop_marker_cycle()
{
   marker_cycle_running_ = false;
   marker_cycle_state_ = MarkerCycleState::Idle;
}
bool CRobot::marker_to_target_from_id(int marker_id,
   double& xt, double& yt,
   double& zt, double& theta_deg)
{
   if (!marker_cam_) {
      std::cout << "[marker_to_target_from_id] marker_cam_ is null.\n";
      return false;
   }

   cv::Vec3d rvec_BM, tvec_BM;
   if (!marker_cam_->get_marker_pose_in_board(marker_id, rvec_BM, tvec_BM)) {
      std::cout << "[marker_to_target_from_id] Cannot find marker "
         << marker_id << " in board frame.\n";
      return false;
   }

   // --- Orientation: same as start_ctraj_to_marker() ---
   cv::Mat Rbm;
   cv::Rodrigues(rvec_BM, Rbm);
   theta_deg =
      std::atan2(Rbm.at<double>(1, 0), Rbm.at<double>(0, 0)) * 180.0 / CV_PI;

   // --- Position: same mapping as start_ctraj_to_marker() ---
   xt = tvec_BM[0];  // board/world X
   yt = tvec_BM[1];  // board/world Y

   // Same Z math as in start_ctraj_to_marker:
   zt = pedestalZ_ + 0.11 + (tvec_BM[2] + tool_z_offset_);

   double r = std::sqrt(xt * xt + yt * yt);
   std::cout << "[marker_to_target_from_id] marker " << marker_id
      << " -> target: x=" << xt
      << " y=" << yt
      << " z=" << zt
      << " (radius=" << r << "), theta=" << theta_deg << " deg\n";

   return true;
}


void CRobot::start_ctraj_cycle(CCameraReal& cam,
   double& q1_deg, double& q2_deg,
   double& q3_deg, double& d3_m)
{
   const float kMarkerLen = 0.02042f;
   ...
   ctraj_cycle_idx_ = 0;   // start at marker 50
   ctraj_cycle_hold_counter_ = 0;
   ctraj_cycle_running_ = true;

   int id = ctraj_cycle_markers_[ctraj_cycle_idx_];

   // Make sure THIS marker's pose is cached this frame
   if (!cam.draw_cube_on_marker(_canvas, id, kMarkerLen, kCubeH)) {
      std::cout << "[ctraj_cycle] Marker " << id
         << " not visible, cannot start.\n";
      ctraj_cycle_running_ = false;
      return;
   }

   // Use your existing ctraj function
   start_ctraj_to_marker(cam, id, q1_deg, q2_deg, q3_deg, d3_m);

   if (!ctraj_running_) {
      std::cout << "[ctraj_cycle] start_ctraj_to_marker failed for marker "
         << id << ", aborting cycle.\n";
      ctraj_cycle_running_ = false;
   }
}

void CRobot::stop_ctraj_cycle()
{
   ctraj_cycle_running_ = false;
}

void CRobot::tick_ctraj_cycle(CCameraReal& cam,
   double& q1_deg, double& q2_deg,
   double& q3_deg, double& d3_m)
{
   if (!ctraj_cycle_running_)
      return;

   const float kMarkerLen = 0.02042f;
   const float kCubeH = 0.030f;

   // While ctraj is running, just let tick_animations / step_ctraj do the work
   if (ctraj_running_)
      return;

   // We only get here once ctraj finished the current leg.
   // Hold for a bit at this marker:
   ctraj_cycle_hold_counter_++;
   if (ctraj_cycle_hold_counter_ < ctraj_cycle_hold_frames_)
      return;

   // Time to move to the next marker
   ctraj_cycle_hold_counter_ = 0;
   ctraj_cycle_idx_ = (ctraj_cycle_idx_ + 1) % 3;
   int id = ctraj_cycle_markers_[ctraj_cycle_idx_];

   std::cout << "[ctraj_cycle] Moving to marker " << id << "\n";

   // Cache pose of the next marker THIS frame
   if (!cam.draw_cube_on_marker(_canvas, id, kMarkerLen, kCubeH)) {
      std::cout << "[ctraj_cycle] Marker " << id
         << " not visible, stopping cycle.\n";
      ctraj_cycle_running_ = false;
