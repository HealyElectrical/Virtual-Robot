...
void CRobot::update_joint_controls(double& q1_deg, double& q2_deg, double& q3_deg, double& d3_m)
{
    ...
    cv::rectangle(_canvas,
        ...
        cv::Point(panel_x + 270, panel_y + 580),
        cv::Scalar(70, 70, 70), cv::FILLED);

    // Window frame
    cvui::window(_canvas, panel_x, panel_y, 260, 550, "SCARA Joint Controls");
    panel_x += 10;
    panel_y += 30;
    ...
    // NOTE: ee_x_mm_, ee_y_mm_, ee_z_mm_, ee_theta_deg_ should be updated
    //       from draw_scara_world() BEFORE calling this function.
    ...
    if (cvui::button(_canvas, panel_x + 130, panel_y, 100, 30, "Reset")) {
        q1_deg = q2_deg = q3_deg = 0.0;
        d3_m = 0.0;
        ee_x_mm_ = ee_y_mm_ = 0.0;
        ee_z_mm_ = 150.0;     // top position (matches spec: reads 150mm at d3=0)
        ee_theta_deg_ = 0.0;
    }
}*/
